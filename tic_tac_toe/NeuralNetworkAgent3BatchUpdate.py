...
class NNAgent:
    ...
    @staticmethod
    def add_layer(input_tensor, output_size, normalize=None, phase=None):
    ...
    @classmethod
    def build_graph(cls):
        NNAgent.phase = tf.placeholder(tf.bool, name='phase')
        ...
        net = cls.add_layer(net, BOARD_SIZE * 3*340, tf.nn.tanh, NNAgent.phase)
        net = cls.add_layer(net, BOARD_SIZE * 3, tf.nn.tanh, NNAgent.phase)
    ...
    def calculate_targets(self):
        ...
        for i in range(game_length - 1, -1, -1):
            ...
            target_action_prob = r + 0.9 * self.next_max_log[i]
    ...
    def move(self, board):
        ...
        probs = NNAgent.sess.run([self.probabilities], feed_dict={self.input_positions: [nn_input],NNAgent.phase : False})[0][0]
    ...
    def final_result(self, result):
        ...
        if TRAINING:
            ...
            # Stochastic training
                ...
                # for (pos, target) in zip(NNAgent.training_data[0], NNAgent.training_data[1]):
                #     self.sess.run([self.train_step],
