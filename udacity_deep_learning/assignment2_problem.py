...
def reformat(dataset, labels):
    ...
    l = labels[:, None]
    ar = np.arange(num_labels)
    # Map 0 to [1.0, 0.0, 0.0 ...], 1 to [0.0, 1.0, 0.0 ...]
    labels = (ar == l).astype(np.float32)
...
with graph.as_default():
    ...
    loss = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(labels=tf_train_labels, logits=l2))
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        ...
        feed_dict = {tf_input_dataset: batch_data, tf_train_labels: batch_labels}

        _, l, predictions = session.run(
            [optimizer, loss, train_prediction], feed_dict=feed_dict)

        if (step % 500 == 0):
            feed_dict = {tf_input_dataset: valid_dataset}
    ...
    feed_dict = {tf_input_dataset: test_dataset}
