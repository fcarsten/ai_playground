...
hidden_units = STATE_SIZE * STATE_SIZE * 25
...
# Generate hidden layer
W1b = tf.Variable(tf.truncated_normal([hidden_units, hidden_units],
                                      stddev=0.1 / np.sqrt(float(STATE_SIZE))))
b1b = tf.Variable(tf.truncated_normal([1, hidden_units]))
h1b = tf.matmul(h1, W1b) + b1b
# h1 = tf.tanh(tf.matmul(input_positions, W1) + b1)
...
class Agent:
    ...
    def move(self, sess, board, training):
        ...
        if training is True and np.random.rand(1) < random_move_prob:
...
def play_game(training=TRAINING):
    ...
    player2 = RandomPlayer(PLAYER_TWO)
    res = DRAW
